...
const parseTransaction = async (bot, msg, { copyWalletAddress }) => {
  ...
  const signatures = await connection.getSignaturesForAddress(
    new PublicKey(copyWalletAddress),
  );
  ...
  const { postTokenBalances } = meta;
  const { preTokenBalances } = meta;
  ...
  const targetTokenBalances = postTokenBalances.filter(
    one => one.owner === copyWalletAddress,
  );
  ...
  const preAmount = preTokenBalances.filter(
    one => one.accountIndex === targetTokenBalances[0].accountIndex,
  )[0].uiTokenAmount.uiAmount;

  const pair = await getPair(targetTokenBalances[0].mint);
  priceSol = parseFloat(pair.priceNative);
  amount = (postAmount - preAmount) * priceSol * 10 ** 9;
